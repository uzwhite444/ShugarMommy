-- ============================================================
-- SUGAR MOMMY — обновление безопасности v3
--
-- ⛔ ИСТОРИЧЕСКИЙ ФАЙЛ. УЖЕ ПРИМЕНЁН. ПОВТОРНО НЕ ВЫПОЛНЯТЬ.
--
-- Всё, что здесь есть, входит в setup-all.sql. Файл оставлен только как
-- история изменений, и запуск закрыт намеренно, по двум причинам:
--
--   1. Ниже (раздел «АНТИ-СПАМ») лежит ДОРЕФОРМЕННАЯ версия
--      bookings_antiflood(): она пропускала любой номер короче 9 цифр,
--      то есть лимит заявок обходился одной запятой в телефоне. Дыра
--      закрыта в 10_antiflood_fix.sql — и повторный прогон этого файла
--      вернул бы её обратно.
--   2. Шапка раньше предлагала менять здесь админа. Смена админа —
--      функция is_admin() в НАЧАЛЕ setup-all.sql; там же про это написано
--      в supabase/README.md.
--
-- Если базу поднимают с нуля: выполняйте setup-all.sql, этот файл не нужен.
-- ============================================================

do $$
begin
  raise exception 'Файл 08_hardening_v3.sql уже применён и повторному запуску не подлежит: он вернёт старую версию bookings_antiflood() с обходом лимита (см. 10_antiflood_fix.sql). Смена админа — is_admin() в setup-all.sql.';
end
$$;

-- ============================================================
-- Ниже — то, что этот файл делал в своё время. Оставлено для истории.
--
-- Что чинил:
...
-- 1. КТО ТАКОЙ АДМИН
--
-- Один аккаунт студии. СМЕНА АДМИНА ДЕЛАЕТСЯ НЕ ЗДЕСЬ: замените uid в
-- is_admin() в начале setup-all.sql и выполните тот файл.
-- ============================================================
...
-- ============================================================
-- БЫЛО: select public.cancel_booking('123', current_date) — вызов функции,
-- которую этот же файл удаляет выше (и которой нет на базе после
-- 11_cancel_by_code.sql). SQL Editor шлёт файл одной транзакцией, поэтому
-- ошибка 42883 в ПОСЛЕДНЕЙ строке откатывала ВЕСЬ файл: перезапуск, который
-- шапка обещает как безопасный, молча не применял ничего.
